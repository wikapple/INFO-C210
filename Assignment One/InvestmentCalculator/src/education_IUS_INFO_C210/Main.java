package education_IUS_INFO_C210;
...
public class Main {
		...
		while(investmentInputLoop) {
			...
			if(in.hasNextInt()) {
				...
				if(initialInvestment > 0 && initialInvestment < 1000000000) {
					System.out.println("Initial investment entered successfully");
	...
	private static double inputInterest() {
		...
		while (interestInputLoop) {
			System.out.println("Enter the annual interest rate as a precentage: ");
	...
	private static int inputYears() {
		...
		while (yearInputLoop) {
			System.out.println("Enter the length of the investment in years: ");
			...
			if(in.hasNextInt()) {
				...
				if(yearsInput > 0 && yearsInput < 1000) {
					System.out.println("Years of investment entered.");
					yearInputLoop = false;
					
				//Else if user input double is too far out of range
				}else {
					System.err.println("Your entry " + yearsInput + " was invalid");
					System.err.println("Please enter an integer for years between 1 and 1000");
				}
